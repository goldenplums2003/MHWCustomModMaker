...
> 如果你拿到过更早的 v3.0 安装包：v3.1 在它之上多修了三处 ——
> 「只发喊话的条目永远不触发」「稍大一点的音频既不能试听也不会响」
> 「立体声重采样会把左右声道搅在一起」。建议直接换成 v3.1。
...
- **判定窗口收得太早，命中被判成落空**：收窗有两条路径 ——「动作结束+余量」
  和「历史最晚出伤+余量」。前者有个坑：**动作结束不等于伤害不会再来**，
  伤害是游戏在命中帧之后才结算的，而动作 ID 可能提前切走。登龙实测有只持续
  125ms 就换 ID 的，加余量在 275ms 就收窗，而 388 次命中的出伤时刻中位数是
  297ms —— 一半以上的命中在窗口关掉之后才到，全被判成落空。全部日志里登龙走
  这条路径收窗 39 次，35 次早于已知出伤窗。现在已经学到「最晚出伤」时，
  这条路径不会比它还早收窗。
...
- **稍大一点的音频既不能试听也不会响**：DLL 卡 16MB、GUI 试听卡 32MB，
  两个数还不一样（32MB 的文件能试听、进游戏却不响）。更麻烦的是超限时
  两边都不说话 —— 日志只有一句 `size invalid`，GUI 点了试听毫无反应，
  没有任何线索指向「文件太大」。
  wav 是不压缩的，44.1kHz 立体声每分钟约 10MB，一段三四分钟的音乐就是 40MB，
  16MB 这个上限对做音频创意来说太紧了。
  现在统一成一个可配置的值（ini 里 `MaxWavMB=`，默认 **64**，范围 1~512），
  超限时两边都写清实际体积、上限和该怎么办。
  （没有取消上限：所有 wav 都常驻内存，配得多配得大就吃多少内存。）
- **立体声重采样会把左右声道搅在一起**：重采样把交错存的 `L R L R…` 当成
  一条平坦的采样流去插值，每个输出点落在一个 L 和一个 R 之间。
  拿「左声道恒 +12000 / 右声道恒 -12000」的信号做 48k→44.1k 实测，
  修之前只有 8.8% 的采样点保住原幅度、50.3% 被拉向 0，立体声整个塌掉。
  单声道两种写法等价所以一直没被发现，但**任何非 44.1kHz 的立体声文件都会中招**。
  现在按「帧」重采样，帧内每个声道各自插值，实测 100% 保真。
