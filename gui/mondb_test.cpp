...
int main(){
    ...
    // 两个 ID 必须用同一个名字 —— 界面按名字去重，挑一次就把两个都填进去；
    // 名字要是不一样，下拉里就成了两行，漏挑一行又回到「看到大咬却没响」。
    Eq("黑龙 28787 (大咬)", LookupMonsterAction("黑龙",28787), "大咬 (3.7s)");
    Eq("黑龙 28788 (大咬)", LookupMonsterAction("黑龙",28788), "大咬 (3.7s)");
    {
        int n = 0;
        for (const auto& e : MonsterActions("黑龙")) if (e.name == "大咬 (3.7s)") ++n;
        Eq("大咬同名的有两条", std::to_string(n), "2");
    }
